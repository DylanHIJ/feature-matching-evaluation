...
def load_seq_npz(seq_dir):
    ref_path = os.path.join(seq_dir, "1.ppm.d2-net")
    ref = np.load(ref_path)
    
    tars = []
    for i in range(5):
        tar_path = os.path.join(seq_dir, f"{i+2}.ppm.d2-net")
...
def _main():
    ...
    for seq_dir in seq_dirs:
        ...
        ref_kps, ref_descrs = ref['keypoints'], ref['descriptors']
        features_nums.append(ref_descrs.shape[0])

        homos = load_seq_homos(seq_dir)
        
        for idx, tar in enumerate(tars):
            tar_kps, tar_descrs = tar['keypoints'], tar['descriptors']
    ...
    mma_dir = os.path.join(args.result_dir, "mma_d2net")
