...
def load_seq_npz(seq_dir):
    ref_path = os.path.join(seq_dir, "1.ppm.r2d2")
    ref = np.load(ref_path)
    
    tars = []
    for i in range(5):
        tar_path = os.path.join(seq_dir, f"{i+2}.ppm.r2d2")
...
def _main():
    ...
    mma_dir = os.path.join(args.result_dir, "mma_r2d2")
