...
from utils.matcher import FlannMatcher, BruteForceMatcher
from utils.metrics import calc_proj_errs

THRESHOLDS = range(1, 11)

def load_seq_npz(seq_dir):
    ref_path = os.path.join(seq_dir, "1.hfnet")
    ref = np.load(ref_path)
    
    tars = []
    for i in range(5):
        tar_path = os.path.join(seq_dir, f"{i+2}.hfnet")
...
def _main():
    ...
    matcher = FlannMatcher(index_params)
    ...
    for seq_dir in seq_dirs:
        ...
        ref_kps, ref_descrs = ref['keypoints'], ref['local_descriptors']
        features_nums.append(ref_descrs.shape[0])

        for idx, tar in enumerate(tars):
            tar_kps, tar_descrs = tar['keypoints'], tar['local_descriptors']
            features_nums.append(tar_descrs.shape[0])
            
            print(tar_descrs.shape)
            print(ref_descrs.shape)
    ...
    mma_dir = os.path.join(args.result_dir, "mma_hfnet")
